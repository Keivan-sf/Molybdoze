...
async function createTables(){
   // await execute(`CREATE TABLE videos(VideoID VARCHAR(12), UserID VARCHAR(40), Title VARCHAR(255), Description TEXT, Likes BIGINT, Dislikes BIGINT, Views BIGINT, Qualities VARCHAR(55), VideoDate DATE)`)
...
/**
 * @description Get keys and values in a row for the sql
 * @example {email : "keyvan0082@gmail.com" , number : 98000000}
 * @param {Object} data Inputs including key:values
...
async function insert(table , data , dateName){
    ...
    console.log(`INSERT INTO ${table} (${keyValues.keys}) VALUES(${keyValues.values})`)

    try{
        const response = await execute(`INSERT INTO ${table} (${keyValues.keys}) VALUES(${keyValues.values})`);
...
module.exports = { runSqlPool , createTables , execute  , queryCreator , insert};