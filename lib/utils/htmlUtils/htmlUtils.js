...
class video_page extends htmlUtils{
    ...
    /**
     * Show video duration before javascript being loaded on the client
     * @param {String} duration 
     */

    setDuration(duration){
        this.htmlPage.index = this.htmlPage.index.replace('[--DURATION]' , duration);
        return this;
    }

    setMetadata(data = {id , quality , tsPreview , subtitle , duration}){

        try{
            let strData = JSON.stringify(data);
            this.htmlPage.index = this.htmlPage.index.replace('[--JSON_MDPL_DATA]' , strData)
        }catch{
            this.htmlPage.index = 'Error';
        }
