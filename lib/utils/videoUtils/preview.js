...
const combineTsShots = (path , frameCounts , resolution) => new Promise(async(resolve , reject) => {
    ...
    const lowFrame = {
        width : Math.round(45 * ratio),
        height : 45
    }

    const lowPreview = {

        width: lowFrame.width * 10,

        height : (function(){

            const devided = frameCounts / 10;
            let height = 0;
            ...
            if(devided === Math.floor(devided)){
                ...
                height = (Math.floor(devided) + 1) * 45;

            }

            if(height > 450){

                const hDevided = height / 450;
                ...
                if(hDevided === Math.floor(hDevided)){
                    ...
                    lowPictureCount = Math.floor(height / 450) + 1;
    ...
    console.log(lowFrame , lowPreview);
    ...
    for(let i=1; i<= frameCounts; i++){
        ...
        let leftOffset = (i-1)%10 * lowFrame.width;
        
        let topOffset = parseInt((i-1) / 10) * lowFrame.height;
        ...
        if(i >= 100){
            ...
            if(i%100 === 0){
                ...
                const targetNumber = (i / 100) -1;

                topOffset = topOffset - (targetNumber * 450);
                ...
                fs.writeFileSync(path + `/low_${targetNumber}.jpg` , lowFinalBuffer);

            }else{
                const targetNumber = Math.floor(i / 100);
    ...
    if(bufferedOutput < lowCanvases.length){
        ...
        await new Promise((resolve , reject)=>{
            ...
            gm(lowFinalBuffer , 'sth,jpg')
            .bitdepth(24)
            .write((path + `/low_${targetNumber}.jpg`) , (error) => {
                if(error) console.log(error)
                resolve((path + `/low_${targetNumber}.jpg`));
            })
   ...
   //     fs.writeFileSync((path + `/low_${targetNumber}.jpg`) , lowFinalBuffer);
    ...
    console.log('here comes the resolve')
