...
module.exports.createTsPreview = async(videoID , mainFile , metaData , qualities) => {
    ...
    const screenPath = `./resources/ts-previews/${videoID}`;
    ...
    if(tn_files.length === 1 && tn_files[0] === 'tn.png') fs.renameSync(`${screenPath}/tn.png` , `${screenPath}/tn_1.png`);
    
    const compined = await combineTsShots(screenPath , screenshotPoints.length , metaData.resolution);

    for(let i=1; i <= tn_files.length; i++) fs.unlinkSync(`./resources/ts-previews/${videoID}/tn_${i}.png`);
